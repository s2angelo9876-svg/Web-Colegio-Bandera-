    ...
    <>
      <nav
        className={`sticky top-0 z-[100] transition-all duration-300 ${
          scrolled
            ? 'bg-white/95 backdrop-blur-md shadow-lg shadow-blue-900/8 border-b border-gray-100'
          ...
          <div className="flex justify-between h-[72px] items-center gap-4">
            ...
            >
              ...
              <div className="flex flex-col leading-none">
                <span className="text-[#003087] font-black text-[17px] tracking-tight uppercase">
                  Bandera del Perú
                </span>
                <span className="text-gray-400 text-[9px] font-bold tracking-[0.22em] uppercase mt-0.5">
            ...
            <div className="hidden lg:flex items-center gap-1 xl:gap-2">
              {navLinks.map((link) => (
                <div key={link.name} className="relative group">
                  {link.submenu ? (
                    <>
                      <button className="flex items-center gap-1 text-gray-600 font-semibold text-[12px] xl:text-[13px] uppercase tracking-wider hover:text-[#003087] transition-colors py-[26px] px-2 xl:px-3 rounded-lg hover:bg-blue-50/60">
                        {link.name}
                        <ChevronDown
                          size={13}
                      ...
                      {/* Dropdown */}
                      <div className="absolute top-full left-0 w-56 bg-white shadow-2xl rounded-2xl p-2 border border-gray-100 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 translate-y-2 group-hover:translate-y-0 animate-slide-down">
                        {link.submenu.map((sub) => {
                          ...
                          return (
                            <Link
                              key={sub.name}
                              to={sub.path}
                              className="flex items-center gap-3 px-4 py-2.5 text-gray-500 hover:text-[#003087] hover:bg-blue-50 rounded-xl text-[12px] font-semibold transition-all group/item"
                            >
                              <span className="w-7 h-7 bg-gray-100 group-hover/item:bg-blue-100 rounded-lg flex items-center justify-center transition-colors flex-shrink-0">
                                <SubIcon size={13} className="text-gray-500 group-hover/item:text-[#003087] transition-colors" />
                              </span>
                              {sub.name}
                            </Link>
                          );
                    ...
                    <Link
                      ...
                      className={`flex items-center gap-1.5 font-semibold text-[12px] xl:text-[13px] uppercase tracking-wider transition-all py-[26px] px-2 xl:px-3 rounded-lg relative ${
                        isActive(link.path)
                          ? 'text-[#003087] bg-blue-50/60'
                          : 'text-gray-600 hover:text-[#003087] hover:bg-blue-50/60'
                      }`}
                    >
                      {link.name}
                      {isActive(link.path) && (
            ...
            <div className="hidden lg:flex items-center gap-2">
              ...
              <button
                ...
                className="w-9 h-9 flex items-center justify-center rounded-xl text-gray-400 hover:text-[#003087] hover:bg-blue-50 transition-all"
                aria-label="Buscar"
              >
