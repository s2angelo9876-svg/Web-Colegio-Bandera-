        ...
        <div className="p-8 lg:p-12 bg-[#F8FAFC] min-h-screen relative overflow-hidden">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-16 relative z-10">
                <div className="animate-in fade-in slide-in-from-left duration-700">
                    ...
                    <h2 className="text-5xl font-black text-gray-900 tracking-tight leading-none mb-4">
                        Agenda <span className="text-[#003087]">Escolar</span>
                    </h2>
                    <p className="text-gray-400 font-bold text-lg">Organiza actividades, ceremonias y fechas cívicas.</p>
                </div>
                <button 
            ...
            {showForm && (
                <div className="animate-in fade-in slide-in-from-top-6 duration-500 mb-12 relative z-10">
                    <form onSubmit={handleSubmit} className="p-10 bg-white/80 backdrop-blur-xl rounded-[3.5rem] shadow-2xl shadow-blue-900/10 border border-white grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="space-y-6 relative z-10">
                {eventos.map(ev => (
                    <div key={ev.id} className="bg-white/70 backdrop-blur-sm p-4 rounded-[3rem] border border-white flex flex-col md:flex-row items-center gap-8 hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 group pr-10 relative overflow-hidden">
                        ...
                        <div className="flex-1 text-center md:text-left py-4 z-10">
                            <h3 className="text-3xl font-black text-gray-900 mb-4 group-hover:text-[#003087] transition-colors tracking-tight">{ev.titulo}</h3>
                            <div className="flex flex-wrap justify-center md:justify-start gap-4">
                                <div className="flex items-center gap-2 text-[11px] font-black text-gray-500 uppercase tracking-widest bg-white/50 backdrop-blur-sm px-5 py-2.5 rounded-2xl border border-gray-100 group-hover:border-blue-100 transition-colors">
                        ...
                        <div className="absolute top-8 right-8 md:relative md:top-auto md:right-auto opacity-0 group-hover:opacity-100 transition-all translate-x-4 group-hover:translate-x-0 z-20">
                            <button 
                                ...
                                className="bg-white text-red-500 p-5 rounded-[1.5rem] hover:bg-red-500 hover:text-white transition-all shadow-xl shadow-red-900/10 border border-red-50"
                            >
                                <Trash2 size={24} />
                            </button>
                        </div>
                ...
                {eventos.length === 0 && (
                    <div className="text-center py-24 bg-white/50 backdrop-blur-sm rounded-[4rem] border border-dashed border-gray-200 shadow-sm mt-6 flex flex-col items-center">
                        ...
                        <p className="text-gray-400 font-black text-xl tracking-tight">No hay eventos registrados.</p>
                        <button onClick={() => setShowForm(true)} className="text-[#003087] font-black text-xs uppercase tracking-widest mt-4 hover:underline">Registrar el primer evento</button>
                    </div>
                )}
            </div>
        </div>
