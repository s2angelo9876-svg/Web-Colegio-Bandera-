    ...
    return (
        <div className="p-8 lg:p-12 bg-[#F8FAFC] min-h-screen relative overflow-hidden">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-16 relative z-10">
                <div className="animate-in fade-in slide-in-from-left duration-700">
                    ...
                    <h2 className="text-5xl font-black text-gray-900 tracking-tight leading-none mb-4">
                        Centro de <span className="text-[#003087]">Comunicados</span>
                    </h2>
                    <p className="text-gray-400 font-bold text-lg">Emite avisos, circulares y alertas urgentes a la comunidad.</p>
                </div>
                <button 
                    ...
                    <form onSubmit={handleSubmit} className="p-10 bg-white/80 backdrop-blur-xl rounded-[3.5rem] shadow-2xl shadow-blue-900/10 border border-white flex flex-col gap-8">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                            <div className="space-y-2 group">
                                <label className="text-[10px] font-black text-gray-400 uppercase ml-4 tracking-widest group-focus-within:text-[#003087] transition-colors">Título del Aviso</label>
                                <input type="text" placeholder="Ej: Suspensión de labores..." className="w-full p-5 rounded-2xl bg-slate-50/50 border border-transparent focus:border-[#003087] focus:bg-white focus:ring-4 focus:ring-blue-50 font-bold text-gray-800 outline-none transition-all" value={titulo} onChange={e => setTitulo(e.target.value)} required />
                            </div>
                            <div className="space-y-2 group">
                                <label className="text-[10px] font-black text-gray-400 uppercase ml-4 tracking-widest group-focus-within:text-[#003087] transition-colors">Tipo de Comunicado</label>
                                <select 
                                    className="w-full p-5 rounded-2xl bg-slate-50/50 border border-transparent focus:border-[#003087] focus:bg-white focus:ring-4 focus:ring-blue-50 font-black text-gray-700 outline-none transition-all cursor-pointer appearance-none" 
                                    value={tipo}
                        ...
                        <div className="space-y-2 group">
                            <label className="text-[10px] font-black text-gray-400 uppercase ml-4 tracking-widest group-focus-within:text-[#003087] transition-colors">Mensaje Oficial Detallado</label>
                            <textarea placeholder="Escribe el contenido detallado aquí..." className="w-full p-6 rounded-[2rem] bg-slate-50/50 border border-transparent focus:border-[#003087] focus:bg-white focus:ring-4 focus:ring-blue-50 font-bold text-gray-600 outline-none transition-all h-48 resize-none text-lg leading-relaxed" value={descripcion} onChange={e => setDescripcion(e.target.value)} required></textarea>
                        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8 relative z-10">
                {comunicados.map(c => (
                    <div key={c.id} className="bg-white/70 backdrop-blur-sm border border-white p-8 rounded-[3rem] shadow-xl shadow-blue-900/5 hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 relative group overflow-hidden flex flex-col h-full min-h-[320px]">
                        ...
                        <div className="absolute top-6 right-6 opacity-0 group-hover:opacity-100 transition-all translate-x-4 group-hover:translate-x-0 z-20">
                            <button onClick={() => handleEliminar(c.id)} className="bg-white text-red-500 p-4 rounded-2xl hover:bg-red-500 hover:text-white transition-all shadow-xl shadow-red-900/10 border border-red-50">
                                <Trash2 size={20} />
                            </button>
                        </div>
                        ...
                        <h3 className="text-2xl font-black text-gray-900 mb-4 leading-tight group-hover:text-[#003087] transition-colors z-10 line-clamp-2 tracking-tight">{c.titulo}</h3>
            ...
            {comunicados.length === 0 && (
                <div className="text-center py-20 bg-white rounded-[3rem] border border-dashed border-gray-200 shadow-sm mt-6">
                    ...
                    <p className="text-gray-400 font-bold text-lg">No hay comunicados publicados actualmente.</p>
                </div>
            )}
        </div>
    )
