...
function AdminAdmisiones() {
    ...
    return (
        <div className="p-8 lg:p-12 bg-[#F8FAFC] dark:bg-slate-900 transition-colors duration-300 min-h-screen relative overflow-hidden">
            ...
            {!cargando && (
                <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8 relative z-10">
                    {admisiones.map(a => (
                        <div key={a.id} className="bg-white/70 dark:bg-slate-800/80 backdrop-blur-sm border border-white dark:border-slate-700 p-8 rounded-[3rem] shadow-xl shadow-blue-900/5 dark:shadow-none hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 relative group overflow-hidden flex flex-col h-full">
                            <div className="absolute -right-8 -top-8 w-24 h-24 bg-blue-50 dark:bg-blue-900/20 rounded-full blur-2xl group-hover:bg-blue-100 dark:group-hover:bg-blue-900/40 transition-colors opacity-50" />
                            ...
                            <div className="absolute top-6 right-6 opacity-0 group-hover:opacity-100 transition-all translate-x-4 group-hover:translate-x-0 z-20">
                                <button onClick={() => handleEliminar(a.id)} className="bg-white dark:bg-slate-700 text-red-500 p-4 rounded-2xl hover:bg-red-500 dark:hover:bg-red-600 hover:text-white transition-all shadow-xl shadow-red-900/10 dark:shadow-none border border-red-50 dark:border-slate-600">
                                    <Trash2 size={20} />
                                </button>
                            </div>
                            
                            <div className="flex items-center gap-3 mb-6 z-10">
                                <span className="text-[10px] font-black uppercase tracking-widest text-gray-400 dark:text-slate-400 flex items-center gap-1.5 bg-slate-50 dark:bg-slate-700/50 px-4 py-2 rounded-xl">
                            ...
                            <div className="mb-6 z-10">
                                ...
                                <p className="text-[10px] font-black text-gray-400 dark:text-slate-400 uppercase tracking-widest flex items-center gap-2 mt-2">
                                    <GraduationCap size={14} /> Postula a: <span className="text-gray-800 dark:text-slate-200">{a.grado_interes}</span>
                                </p>
                            </div>

                            <div className="bg-slate-50 dark:bg-slate-700/30 p-6 rounded-2xl mb-2 flex-1 z-10 border border-slate-100 dark:border-slate-700">
            ...
            {!cargando && admisiones.length === 0 && (
                <div className="text-center py-20 bg-white dark:bg-slate-800 rounded-[3rem] border border-dashed border-gray-200 dark:border-slate-700 shadow-sm mt-6">
                    <FileText size={64} className="mx-auto text-gray-200 dark:text-slate-600 mb-4" />
